                              ...
                              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 sm:gap-6">
                                {groupedSkills[category].map((skill, skillIndex) => {
                                  ...
                                  return (
                                    <div
                                        key={skill.name}
                                        className={`flex flex-col items-center justify-center gap-3 p-4 sm:p-6 bg-slate-800/20 border border-slate-700/50 rounded-xl transition-all duration-150 ease-in-out hover:bg-slate-700/40 hover:border-blue-500/50 hover:-translate-y-1.5 transform ${isActive ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}
                                        style={{ transitionDelay: `${isActive ? cardDelay : 0}ms` }}
                                    >
                                        <div className="text-slate-300 transition-colors duration-150">{iconMap[skill.icon] || <CodeBracketIcon className="w-10 h-10" />}</div>
